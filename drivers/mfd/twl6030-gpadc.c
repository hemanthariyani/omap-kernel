...
static struct twl6030_calibration
...
struct twl6030_gpadc_data {
	...
	int irq_n;
	struct twl6032_calibration *twl6032_cal_tbl;
					  ...
					  u8 reg)
{
	...
	if (gpadc->features & PHOENIXLITE_SUBCLASS) {
		...
		u8 reg_base, u32 channels, struct twl6030_gpadc_request *req)
{
	...
	if (gpadc->features & PHOENIXLITE_SUBCLASS)
		nochans = TWL6032_GPADC_MAX_CHANNELS;
	else
		nochans = TWL6030_GPADC_MAX_CHANNELS;

	for (i = 0; i < nochans; i++) {
		if ((channels & (1<<i)) && (gpadc->features &
				PHOENIXLITE_SUBCLASS)) {
			dev_dbg(gpadc->dev, "GPADC chn: %d", i);
...
int twl6030_gpadc_conversion(struct twl6030_gpadc_request *req)
{
	...
	if (the_gpadc->features & PHOENIXLITE_SUBCLASS)
...
static int __devinit twl6030_gpadc_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "GOT HERE2");

	if (pdata->features & PHOENIXLITE_SUBCLASS) {
		gpadc->twl6032_cal_tbl = kzalloc(
				sizeof(struct twl6032_calibration) *
