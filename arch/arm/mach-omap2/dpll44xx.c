...
int omap4460_mpu_dpll_set_rate(struct clk *clk, unsigned long rate)
{
	...
	/*
	 * On OMAP4460 and OMAP4470, to obtain MPU DPLL frequency
	 * higher than 1GHz, DCC (Duty Cycle Correction) needs to
	 * be enabled. And needs to be kept disabled for < 1 Ghz.
	 *
	...
	if (cpu_is_omap446x() || (cpu_is_omap447x() && rate <= OMAP_1GHz)) {
		...
		/*
		 * On 4470, the MPU clk for frequencies higher than 1Ghz
